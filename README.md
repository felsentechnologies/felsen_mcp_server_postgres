...
Keep require_approval enabled for execute_dml, execute_ddl, and execute_script. See the [OpenAI MCP API documentation](https://developers.openai.com/api/docs/mcp) and [MCP server guidance for plugins](https://developers.openai.com/plugins/build/mcp-server).
...
counts. Use follow-up read-only `COUNT(*)` queries to certify final table counts;
